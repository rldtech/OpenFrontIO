...
import { AllPlayers,  Cell, Execution, MutableGame, MutablePlayer, MutableUnit, Player, PlayerID, Tile, Unit, UnitType, UnitTypes } from "../game/Game";
...
export class PortExecution implements Execution {
    ...
    tick(ticks: number): void {
        if (this.port == null) {
            ...
            if (!new BuildValidator(this.mg).canBuild(this.player, tile, UnitTypes.Port)) {
            ...
            this.port = this.player.addUnit(UnitTypes.Port, 0, spawns[0])
            this.player.removeGold(UnitTypes.Port.cost)
        }
        ...
        const allPorts = this.mg.units(UnitTypes.Port)
