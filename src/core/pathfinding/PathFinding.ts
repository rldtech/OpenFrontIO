...
export class PathFinder {
    ...
    public static Mini(game: Game, iterations: number, canMove: (t: TerrainTile) => boolean, maxTries: number = 20) {
        ...
        return new PathFinder(
            ...
            (curr: Tile, dst: Tile) => {
                return new SerialAStar(
                    game.terrainMap().terrain(curr.cell()),
                    dst.terrainTile(),
                    canMove,
                    iterations,
                    maxTries
    ...
    public static Parallel(game: Game, worker: WorkerClient, numTicks: number, ...terrainTypes: TerrainType[]): PathFinder {
        if (terrainTypes.length == 0) {
            terrainTypes = [TerrainType.Ocean]
        }
        return new PathFinder(
            game,
            (curr: Tile, dst: Tile) => {
                return worker.createParallelAStar(curr, dst, numTicks, terrainTypes)
            }
        )
    }
